...
long * vs_square_1_svc(long *argp, struct svc_req *rqstp) {
	long  *result = malloc(sizeof(long));
	*result = *argp * *argp;
	return result;
}

long * vs_add_1_svc(add_params *argp, struct svc_req *rqstp) {
	long  *result = malloc(sizeof(long));
	*result = argp->p1 + argp->p2;
	return result;
}

char ** vs_concat_1_svc(concat_params *argp, struct svc_req *rqstp) {
	static char * result;
	//xdr_free((xdrproc_t) char*, (char* ) result);
...
split_return * vs_split_1_svc(char **argp, struct svc_req *rqstp) {
	static split_return result;

	int pre, post = 0;

	// Check for split character position
...
void * vs_increment_1_svc(long *argp, struct svc_req *rqstp) {
	static long v = 0;
	v += *argp;
	return NULL;
}
