...
public class Client {
    ...
    private static void call_basic(String namespace, int port, int c) {
        Registry registry = null;
        BasicIf basic = null;

        try {
            registry = LocateRegistry.getRegistry(port);
            basic = (BasicIf) registry.lookup(  "a4-" + namespace  );
    ...
    private static void call_extended(String namespace, int port, int c) {
        Registry registry = null;
        ExtendedIf extended = null;

        try {
            registry = LocateRegistry.getRegistry(port);
            extended = (ExtendedIf) registry.lookup( "a4-" + namespace );
        } catch (RemoteException | NotBoundException e) {
    ...
    public static void main(String[] args) {
        Client.call_basic("basic", 2500, 1);
